...
client.on("message", async message => {
  ...
  try {
    ...
    client.users.cache.forEach(async user => {
      const sgvew = require("./model/money");
      const pqow = require("./model/banco");

      const dapksoa = await sgvew.findOne({
        userId: user.id,
        guildId: message.guild.id
      });
      const sqjgv = await pqow.findOne({
        userId: user.id,
        guildId: message.guild.id
      });
...
[link](https://top.gg/bot/779918057027665960)`
      ...
      {
        if (cooldown2.has(user))
          return message.channel.send(
            "Hey! No puedes trabajar de nuevo hasta dentro de **30 minutos** !"
          );
        else {
          cooldown2.add(user);
        }
        setTimeout(() => {
          cooldown2.delete(user);
        }, ms("30m"));
      }

      const moneyUserServer = await money.findOne({ userId: user });

      let cacthElements = Math.floor(Math.random() * 300) + 50;

      let jobs = [
        "Trabajas en una cafeteria y ganaste $",
        "Trabajaste de repartidor de pizas y recibiste $",
        "Editaste un video de un YouTuber y te pago $"
      ];
      let cacthJobs = jobs[Math.floor(jobs.length * Math.random())];

      let currentTotalMoney = parseInt(moneyUserServer.dinero);

      let moreMoney = parseInt(cacthElements);

      moneyUserServer.dinero = currentTotalMoney + moreMoney;
      await moneyUserServer.save(err => {
        if (err) return console.error(err);
      });

      message.channel.send(`> ${cacthJobs}${cacthElements}`);
    } else if (command === "bal") {
      const money = require("./model/money");
      const bank = require("./model/banco");
      ...
      if (miembro.user.bot)
        return message.channel.send("Los bots no tienen balance!");

      let actualDinero = await money.findOne({ userId: miembro.user.id });
      let actualBanco = await bank.findOne({ userId: miembro.user.id });

      const embed = new MessageEmbed()
        .setTitle(`Balance de ${miembro.displayName}`)
        .setColor("RANDOM")
        .addField(":money_with_wings: Dinero", actualDinero.dinero + "$")
        .addField(":bank: Banco", actualBanco.banco + "$")
        .setTimestamp();
      ...
      let dinero = await money.findOne({ userId: message.author.id });

      let bank = await banco.findOne({ userId: message.author.id });

      if (!args[1])
        return message.channel.send(
          "Debes poner un monto a depositar, opciones: (numero)/all"
        );

      if (args[1] == "all") {
        if (dinero.dinero <= 0)
          return message.channel.send("No tienes dinero para depositar");

        let actualBanco = parseInt(bank.banco);

        let actualDinero = parseInt(dinero.dinero);

        await bank.updateOne({
          userId: message.author.id,
          guildId: message.guild.id,
          banco: actualBanco + actualDinero
        });
        ...
        message.channel.send(`Has depositado todo tu dinero al banco!`);
      } else {
        ...
        let deposit = parseInt(args[1]);

        if (deposit <= 0)
          return message.channel.send("No puedes depositar menos o igual a 0");

        if (dinero.dinero < deposit)
          return message.channel.send(
        ...
        await dinero.updateOne({
          userId: message.author.id,
          guildId: message.guild.id,
          dinero: actualDinero - deposit
        });

        message.channel.send(`Has depositado $ ${args[1]} al banco!`);
      }
      ...
      if (!args[1])
        return message.channel.send(
          "Debes poner un monto a sacar, opciones: (numero)/all"
        );

      if (args[1] == "all") {
        if (bank.banco == 0)
          return message.channel.send(
            "No tienes dinero en el banco para sacar"
          );
        ...
        await dinero.updateOne({
          userId: message.author.id,
          guildId: message.guild.id,
          dinero: actualBanco + actualDinero
        });
        ...
        if (bank.banco <= 0)
          return message.channel.send("No tienes dinero en el banco");
        ...
        if (deposit <= 0)
          return message.channel.send("No puedes sacar menos o igual a 0");

        if (bank.banco < deposit)
          return message.channel.send(
            "No puedes sacar mas de lo que tienes en el banco"
          );
        ...
        message.channel.send(`Has sacado $ ${args[1]} del banco!`);
      }
    } else if (command == "crime") {
      let money = require("./model/money");

      let dinero = await money.findOne({ userId: message.author.id });

      let crimenes = [
        "Has robado una joyeria y escapaste con éxito, ganaste $",
        "Has hackeado a alguien en minecraft y nadie se dio cuenta, ganaste $",
        "Le has robado la targeta de credito a tu mama y compraste un videojuego, ganaste $",
        "Pensaste robar una tienda de ropa pero te atraparon, perdiste $",
        "Robaste una tienda de pasteles y te escapaste con el auto, te caiste en un pozo, perdiste $",
        "Te pusiste a bucear para buscar oro, te lastimaste mucho con una piedra espinosa, pagaste $"
      ];
      ...
      let randomCrimenes =
        crimenes[Math.floor(Math.random() * crimenes.length)];

      let randomDinero = Math.floor(Math.random() * 250) + 30;

      if (crimenesBien.includes(randomCrimenes)) {
        const embed = new MessageEmbed()
          .setTitle("Crimen")
          .setDescription(`${randomCrimenes}${randomDinero}`)
          .setColor("ff0000");

        await dinero.updateOne({
          guildId: message.guild.id,
          userId: message.author.id,
          dinero: parseInt(dinero.dinero) + randomDinero
        });
        return message.channel.send(embed);
      }
      if (crimenesMal.includes(randomCrimenes)) {
        const embed = new MessageEmbed()
          .setTitle("Crimen")
          .setDescription(`${randomCrimenes}${randomDinero}`)
          .setColor("ff0000");
      ...
      if (!mencion)
        return message.channel.send("Debes mencionar a un miembro para robar!");

      let dinero = await money.findOne({ userId: mencion.user.id });
      let dineroUser = await money.findOne({ userId: message.author.id });

      if (mencion == message.member)
        return message.channel.send("No puedes robarte a vos mismo!");
      ...
      if (dinero.dinero == 0) {
        const embed = new MessageEmbed()
          ...
          .setDescription(
            "Quisiste robar a " +
              mencion.user.username +
              ", pero no tenía dinero en mano, intentalo mas tarde"
          )
          .setColor("ff0000");
        return message.channel.send(embed);
      }
      let randoms = [
        "Has entrado por la ventana de su casa y le robaste una billetera, has ganado $",
        "Has entrado a su minecraft y le has destruido todo, ganaste $",
        "Has descubierto su canal de youtube y lo compartiste con todos, ganaste $",
        "Intentaste robarle por el bolsillo pero te descubrió, perdiste $",
        "Le robaste un cubo rubik pero tenía pinches y te lastimaste grave, pagaste $",
        "Quisiste robarle el wifi pero solo conseguiste que pagaras el wifi, pagaste $"
      ];
      let randomCrimen = randoms[Math.floor(Math.random() * randoms.length)];

      let randomDinero = Math.floor(Math.random() * 350) + 30;
      ...
      if (
        [
          ...
          "Has entrado a su minecraft y le has destruido todo, ganaste $",
          "Has descubierto su canal de youtube y lo compartiste con todos, ganaste $"
        ].includes(randomCrimen)
      ) {
        const bien = new MessageEmbed()
          .setTitle("Robo")
          .setDescription(`${randomCrimen}${randomDinero}`)
          .setColor(exito_color);

        await dineroUser.updateOne({
          userId: message.author.id,
          guildId: message.guild.id,
          dinero: parseInt(dineroUser.dinero) + parseInt(randomDinero)
        });
        await dinero.updateOne({
          userId: mencion.user.id,
          guildId: message.guild.id,
          dinero: parseInt(dinero.dinero) - parseInt(randomDinero)
        });
        return message.channel.send(bien);
      }
      if (
        [
          "Intentaste robarle por el bolsillo pero te descubrió, perdiste $",
